...
#ifndef __VERTICAL_SEG_HPP__
#define __VERTICAL_SEG_HPP__
...
public:
    ...
    std::vector<std::pair<int8_t, int8_t> > m_v_vertical_neighbor_iter; // neighbor iterator for segmentaiton process
    ...
    std::vector<std::vector<bool>> m_v_b_vertical_segmented_image;
...
public:
    ...
    void LabelHorizontalComponents(CRangeImage& range_image, uint16_t row, uint16_t col);
    void LabelVerticalComponents(CRangeImage& range_image, uint16_t row, uint16_t col);
    void Reset();
    bool ParseIni();
};
#endif //__VERTICAL_SEG_HPP__