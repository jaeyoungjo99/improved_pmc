...
public:
        ...
        for (int x_neighbor = -max_range; x_neighbor <= max_range; x_neighbor++)
        {
            for (int y_neighbor = -max_range; y_neighbor <= max_range; y_neighbor++)
            {
                for (int z_neighbor = -max_range; z_neighbor <= max_range; z_neighbor++)
                {
                    ...
                    if (neighbor_dis - (float) max_range < 0.001f) // 직각 방향으로만 확장하게 해준다
                    {
    ...
    void extendVoxelNeighborHeight(int voxel, std::unordered_set<int> &voxel_added, float & max_height, float & min_height, int & i_cur_z)
    {
        if(max_height == FLT_MAX){
            max_height = 0.0;
            min_height = 0.0;
            i_cur_z = 0;
        }
                ...
                for (int z_neighbor = -max_range; z_neighbor <= max_range; z_neighbor++)
                {
                    ...
                    if (neighbor_dis - (float) max_range < 0.001f) // 직각 방향으로만 확장하게 해준다
                    {
                        ...
                        {   
                            ...
                            i_cur_z += z_neighbor;
                            if(i_cur_z * Voxel_revolusion > max_height) max_height = i_cur_z * Voxel_revolusion;
                            if(i_cur_z * Voxel_revolusion < min_height) min_height = i_cur_z * Voxel_revolusion;

                            
                            extendVoxelNeighborHeight(voxel_neighbor, voxel_added, max_height, min_height, i_cur_z);
                        }
                    }
                }
            }
    ...
    {   
        ...
        // voxel_list: voxel position에 대한 list
        // voxel_ind: voxel_list의 인덱스 (position)
        for (int voxel_ind = 0; voxel_ind < voxel_list.size(); voxel_ind++)
        {   
            int voxel_cur = voxel_list[voxel_ind]; // voxel position
    ...
    void extractReverse(std::vector<std::vector<int>> &voxel_clusters)
    {   
        // std::vector<Point_Cloud> umap_copy = umap;
        // std::unordered_set<int> voxel_set_copy = voxel_set;
        // voxel_list: voxel position에 대한 list
        // voxel_ind: voxel_list의 인덱스
        for (int voxel_ind = 0; voxel_ind < voxel_list.size(); voxel_ind++)
        {   
            int voxel_cur = voxel_list[voxel_ind]; // voxel position
            if (voxel_set.count(voxel_cur))
    ...
    void extractReverseHeight(std::vector<std::vector<int>> &voxel_clusters, float height_thr)
    {   
        // std::vector<Point_Cloud> umap_copy = umap;
        // std::unordered_set<int> voxel_set_copy = voxel_set;
        // voxel_list: voxel position에 대한 list
        ...
        {   
            ...
            if (voxel_set.count(voxel_cur))
            {
                ...
                int temp_z = 0;
                extendVoxelNeighborHeight(voxel_cur, voxel_added, max_height, min_height, temp_z);
                int size = 0;
                std::vector<int> voxel_candidate_vec;
                for(auto iter=voxel_added.begin(); iter!=voxel_added.end(); ++iter)
                {
                ...
                if (size < min_cluster_voxels_ || (max_height - min_height) < height_thr)
                {
                    // std::cout << "---------------";
                    voxel_clusters.push_back(voxel_candidate_vec);
                }
                else{
                    std::cout<<"Height: "<<max_height - min_height<<std::endl;
                }
