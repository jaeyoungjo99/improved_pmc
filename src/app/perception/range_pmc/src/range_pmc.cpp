...
{   
    ...
    for (auto i : index)
    {   // 한 range image 내부를 iteration
        // if(p[i].ver_ind > pixel_fov_up || p[i].ver_ind < pixel_fov_down || \
        //    p[i].hor_ind > pixel_fov_left || p[i].hor_ind < pixel_fov_right ){
        //     continue;
        // }
        ...
        // p[i].reset(); // point 한개 초기화. p[i]는 포인터가 아니라 객체임  
        V3D p_body(feats_undistort->points[i].x, feats_undistort->points[i].y, feats_undistort->points[i].z); // 라이다 기준 point 위치
        V3D p_glob(rot_end * (p_body) + pos_end); // global point 위치 
        
        // 0304
        // std::cout<<"p_body: "<<p_body<<" p_glob: "<<p_glob<<std::endl;
        // p[i].glob = p_glob;        
        // p[i].dyn = STATIC;
        // p[i].rot = rot_end.transpose();
        // p[i].transl = pos_end;
    ...
    if(is_key_frame == true){
        // GenerateRangeImage(points, scan_end_time, rot_end, pos_end); // 0304
...
bool RangePmc::Case1(point_soph & p)
{
    ...
    // if(valid_image <  params_.i_min_occluded_num) p.dyn = INVALID;
...
{   
    ...
    // std::cout<<"min_range: " <<min_range<<" cur_range: "<<p.vec(2)<< std::endl;
...
void RangePmc::GenerateRangeImage(std::vector<point_soph*> &points, double cur_time, M3D rot, V3D transl)
{   
    double t00 = omp_get_wtime();
    ...
    std::cout<<"[GenerateRangeImage] len "<<len<<std::endl;
    

    if(range_image_list.size() < params_.i_max_range_image_num){
        map_index ++;
        RangeImage::Ptr new_image_pointer(new RangeImage(rot, transl, cur_time, map_index));
        range_image_list.push_back(new_image_pointer);
    }
    else{
        map_index ++;
        ...
        RangeImage::Ptr new_image_pointer = range_image_list.front();
        range_image_list.pop_front();
        range_image_list.push_back(new_image_pointer); // 메모리 교체 
    }

    std::cout<<"[GenerateRangeImage] New image Generated"<<std::endl;
    ...
    { 
        ...
        if(range_image_list.back()->range_image[points[k]->position].size() < max_pixel_points)
        {   
            range_image_list.back()->range_image[points[k]->position].push_back(points[k]);


            // 이 포인트의 range가 픽셀 내 모든 포인트보다 크다면 
            if (points[k]->vec(2) > range_image_list.back()->max_range_all[points[k]->position])  
            {
                range_image_list.back()->max_range_all[points[k]->position] = points[k]->vec(2);
                range_image_list.back()->max_range_index_all[points[k]->position] = range_image_list.back()->range_image[points[k]->position].size()-1;
            }

            // 이 포인트의 range가 픽셀 내 모든 포인트보다 작다면, 혹은 min_range_all가 0이라면
            if (points[k]->vec(2) < range_image_list.back()->min_range_all[points[k]->position] ||\
                range_image_list.back()->min_range_all[points[k]->position] < 10E-5)  
            {
                range_image_list.back()->min_range_all[points[k]->position] = points[k]->vec(2);
                range_image_list.back()->min_range_index_all[points[k]->position] = range_image_list.back()->range_image[points[k]->position].size()-1;
            }
...
{   
    ...
    { 
        ...
        if(range_image_pointer->range_image[points[k].position].size() < max_pixel_points)
        {   
            range_image_pointer->range_image[points[k].position].push_back(&points[k]);
...
{   
    ...
    V3D u_point, l_point;
    double v_angle, i_angle; // vertical, incident angle
    ...
    for(int hor_idx = pixel_fov_right; hor_idx <= pixel_fov_left; hor_idx++)
    {
        ...
        {   
            ...
            std::cout<<"u_point_min_pos: "<<u_point_min_pos<<" "<<l_point_min_pos<<std::endl;
            ...
            v_angle = abs(atan2(ver_dist,hor_dist));

            if(v_angle < params_.f_ground_angle){
...
void RangePmc::SphericalProjection(point_soph &p, int range_index, const M3D &rot, const V3D &transl, point_soph &p_spherical)
{
    ...
    // if(fabs(p.last_vecs.at(range_index%HASH_PRIM)[2]) > 10E-5)
    // {       
    //     p_spherical.vec = p.last_vecs.at(range_index%HASH_PRIM);
    //     p_spherical.hor_ind = p.last_positions.at(range_index%HASH_PRIM)[0];
    //     p_spherical.ver_ind = p.last_positions.at(range_index%HASH_PRIM)[1];
    //     p_spherical.position = p.last_positions.at(range_index%HASH_PRIM)[2];     
    // }
    // else
    // {
    //     V3D p_proj(rot*(p.glob - transl));
    //     p_spherical.GetVec(p_proj, hor_resolution_max, ver_resolution_max);
    //     p.last_vecs.at(range_index%HASH_PRIM) = p_spherical.vec;
    //     p.last_positions.at(range_index%HASH_PRIM)[0] = p_spherical.hor_ind;
    //     p.last_positions.at(range_index%HASH_PRIM)[1] = p_spherical.ver_ind;
    //     p.last_positions.at(range_index%HASH_PRIM)[2] = p_spherical.position;
    // }
...
{   
    ...
    for (int i = -n; i <= n; i++)
    {
        for (int j = -n; j <= n; j++)
        {
            ...
            {
                // float cur_max_range = image_info.max_range_static[cur_pos];
                // float cur_min_range = image_info.min_range_static[cur_pos];
